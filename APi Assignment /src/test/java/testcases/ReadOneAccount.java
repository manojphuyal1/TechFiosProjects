	...
	// return bearerToken; 
	  ...
	  public void readOneAccount() {
		  ...
		  JsonPath jp=new JsonPath(responseBody);
			...
			Assert.assertEquals(accountName,"MD Techfios account 1","Account Name doesnt match");
			
			String accountNumber=jp.getString("account_number");
			Assert.assertEquals(accountNumber,"123456","Account Number doesnt match");
			...
			Assert.assertEquals(balance,"10.22","Balance doesnt match");
			
			String contactPerson=jp.getString("contact_person");
			Assert.assertEquals(contactPerson,"MD Islam","contactPerson doesnt match");
